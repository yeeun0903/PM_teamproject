...
/* ───── 가로 전환 아이콘 ───── */
function LandscapeIcon({ size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2">
...
export default function Detail() {
  ...
  useEffect(() => {
    ...
    const swapAt = content.banners[1].swapAt * 1000;
    ...
    timers.push(setTimeout(() => setBannerIndex(1), swapAt));
  ...
  return (
    <div style={{ background: '#111', minHeight: '100vh', paddingBottom: 80, color: '#fff' }}>
      ...
      <video
        ...
        style={{ width: '100%', height: '100%', objectFit: isFullscreen ? 'contain' : 'cover', filter: `brightness(${brightness}%)` }}
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.35)' }}
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
              ...
              <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                {/* 전체화면 */}
                <button id="btn_detail_fullscreen" onClick={() => { trackClick('btn_detail_fullscreen', { isFullscreen }); toggleFullscreen(); }} style={{ background: 'none', border: 'none', color: '#fff' }}>
                ...
                {/* 캐스트 */}
                <button id="btn_detail_cast" onClick={() => { trackClick('btn_detail_cast'); showToast('캐스트 준비 중'); }} style={{ background: 'none', border: 'none', color: '#fff' }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2">
                    <path d="M2 8.5A12.5 12.5 0 0 1 14.5 21"/><path d="M2 13a8 8 0 0 1 8 8"/>
                    <rect x="2" y="17" width="4" height="4" rx="1"/>
                    <path d="M2 3.5A18.5 18.5 0 0 1 20.5 22"/>
                  </svg>
                </button>
                {/* 더보기 */}
            ...
            {/* 좌측 밝기 */}
            <div style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              ...
              <input
                ...
                style={{ writingMode: 'vertical-lr', direction: 'rtl', height: 80, accentColor: '#fff' }}
              />
            </div>
            ...
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', display: 'flex', alignItems: 'center', gap: 36 }}>
              ...
              <button id="btn_player_play" onClick={() => { trackClick('btn_player_play', { isPlaying }); togglePlay(); }} style={{ background: 'none', border: 'none', color: '#fff' }}>
                {isPlaying
                  ? <svg width="44" height="44" viewBox="0 0 24 24" fill="#fff"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>
                  : <svg width="44" height="44" viewBox="0 0 24 24" fill="#fff"><polygon points="5,3 19,12 5,21"/></svg>
                }
              </button>
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 14px 10px' }}>
              ...
              <input type="range" min={0} max={duration || 100} value={currentTime}
                ...
                style={{ width: '100%', marginBottom: 10, accentColor: '#1A6DFF' }}
              />
              {/* 하단 버튼 행 */}
          ...
          <div style={{ position: 'absolute', bottom: showControls ? 54 : 12, right: 8, zIndex: 10 }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.6)' }}>광고</span>
                {canClose && (
                  <button
                    id="btn_ad_close"
              ...
              <button
                id="btn_ad_banner"
                onClick={() => { trackClick('btn_ad_banner', { bannerIndex, url: content.banners[bannerIndex].url }); window.open(content.banners[bannerIndex].url, '_blank'); }}
                style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', display: 'block' }}
              >
                <img
                  src={content.banners[bannerIndex].img}
                  alt="광고"
